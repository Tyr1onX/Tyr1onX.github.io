...
static uint64_t filetime_100ns(){ FILETIME f{}; GetSystemTimeAsFileTime(&f); ULARGE_INTEGER u{}; u.LowPart=f.dwLowDateTime; u.HighPart=f.dwHighDateTime; return u.QuadPart; }
...
int main(int argc,char**argv){
    ...
    const DWORD waitBeforeMs=argc>=8?(DWORD)std::strtoul(argv[7],nullptr,10):0;
    ...
    std::cout<<"ready_pid="<<GetCurrentProcessId()<<" wait_before_ms="<<waitBeforeMs<<"\n"<<std::flush;
    if(waitBeforeMs){
        const uint64_t until=real_nt_ms()+waitBeforeMs;
        while(real_nt_ms()<until) SwitchToThread();
    }

    if(!useHook){
        HMODULE ext=GetModuleHandleA("speedpatch64.dll");
        if(ext){
            getSpeed=(GetSpeedFn)GetProcAddress(ext,"SP_GetSpeed");
            isEnabled=(IsEnabledFn)GetProcAddress(ext,"SP_IsEnabled");
        }
    }

    // Drain after the external-injection window so that waiting cannot create
    // an artificial startup burst. Measurement starts from the same residual.
    refill(total);
    for(;;){
    ...
    const uint64_t r0=real_nt_ms(),f0=filetime_100ns(),k0=nowk();
    while(got<target){
        refill(total);
    ...
    FlushFileBuffers(fh); const uint64_t k1=nowk(),f1=filetime_100ns(),r1=real_nt_ms(); CloseHandle(fh); DeleteFileA(tmpFile);
    ...
    std::cout<<"enabled="<<(isEnabled && isEnabled()?1:0)
      <<" factor="<<std::fixed<<std::setprecision(2)<<(getSpeed?getSpeed():1.0)
      ...
      <<" filetime_over_real="<<std::setprecision(3)<<(rm?double(f1-f0)/(double(rm)*10000.0):0.0)
